...
insert(X, node(X, Lt, Rt), node(X, Lt, Rt)).

insert(X,node(E,L,R),node(E,L2,R)) :- X < E, 
	insert(X,L,L2).

insert(X,node(E,L,R),node(E,L,R2)) :- X > E,
	insert(X,R,R2).
...
to_list(node(X,nil,nil),[X]).
to_list(node(E,L,nil),X) :-
	to_list(L,L2),
	append(L2,[E],X).
to_list(node(E,nil,R),X) :-
	to_list(R,R2),
	append([E],R2,X).
to_list(node(E,L,R),X) :-
	to_list(L,L2),
	append(L2,[E],Xs),
	to_list(R,R2),
	append(Xs,R2,X).
...
right_rotate(node(Z,node(E,L,R),R2), node(E,L,node(Z,R,R2))).
...
drop_level(_,0,nil).
drop_level(node(E,_,_),1,node(E,_,_)).
drop_level(node(E,L,R),N,node(E,L2,R2)) :- N2 is N-1,	
	drop_level(L,N2,L2),
	drop_level(R,N2,R2).	
...
cons_list(nil,[]).
cons_list(cons(v,L,R),[L|R2]) :- cons_list(R,R2). 
cons_list(cons(c,L,R),Y) :-
	cons_list(L, L2),
	cons_list(R, R2),
	append([L2],R2,Y).
